...
function MessageBubble({ msg, prevMsg, onReact, onPin }) {
  ...
  return (
    <div
      ...
      style={{ display: "flex", gap: 10, padding: `${showAvatar ? 10 : 2}px 20px`, position: "relative", transition: "background 0.1s", background: hover ? C.elevated + "80" : "transparent" }}
    >
      {/* Avatar column */}
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ fontSize: 13.5, color: C.text, lineHeight: 1.55, wordBreak: "break-word" }}>{msg.content}</div>
...
export default function CommsCenter() {
  ...
  const filteredGroups = groupConvs.filter(c => c.name.toLowerCase().includes(search.toLowerCase()));
      ...
      <div style={{ width: 268, flexShrink: 0, background: C.surface, borderRight: `1px solid ${C.border}`, display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ padding: "16px 16px 12px", borderBottom: `1px solid ${C.border}` }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
            <div style={{ width: 32, height: 32, borderRadius: 9, background: `linear-gradient(135deg, ${C.orange}, #c2410c)`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 17, fontWeight: 800, color: "#fff" }}>⚡</div>
            <div>
              <div style={{ fontSize: 14, fontWeight: 800, color: C.text, letterSpacing: -0.3 }}>ClawForge</div>
              <div style={{ fontSize: 9, color: C.textMuted, fontWeight: 500, letterSpacing: 1, textTransform: "uppercase" }}>Comms Center</div>
            </div>
            {totalUnread > 0 && (
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "10px 8px" }}>
          ...
          {filteredDMs.map(c => (
            <ConvItem key={c.id} conv={c} active={activeId === c.id} onClick={() => selectConv(c.id)} />
          ))}
